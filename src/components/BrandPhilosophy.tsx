  ...
  return (
    <div className="flex justify-between flex-wrap md:flex-row  md:text-start text-center flex-col-reverse items-center">
      ...
      <div className="md:w-[45%]">
        <p className="md:text-2xl text-base text-primary mb-3">Our Value</p>
        ...
        <div>
          {BrandPhilosophyData.map((item, index) => {
            return (
              <div className="border border-[#557DB836] py-[26px] px-[35px] text-start md:mb-[49px] mb-[20px]" key={index}>
                <div className={`flex justify-between items-center ${dropDownOpen === item.id && "mb-6"}`}>
                  <div className="flex items-center">
                    <div className="w-[39px] sm:mr-12 mr-2 h-[26px] bg-primary3"></div>
                    <p className="sm:text-base text-sm">{item.label}</p>
                  </div>
                  <BsCaretDownSquareFill
                    className="text-primary3 w-[34px] h-[26px] cursor-pointer"
                    onClick={() => {
